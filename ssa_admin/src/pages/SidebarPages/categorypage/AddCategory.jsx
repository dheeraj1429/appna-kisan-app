...
const handleSubmit = async (e)=>{
 ...
 mainCategoryImageToFirebase = selectMainCategory?.find(value=>value?._id === mainCategory)
  if(fileUpload.categoryImage ){
    categoryImageToFirebase=await uploadFileToFirebase(`/ssastore/allcategories/categories/${category}/` ,fileUpload.categoryImage)
  }
  let subCategoryImageToFirebase =[]
  for(let i=0;i<subCategoryField.length;i++){
    if(subCategoryField[i].subCategoryImage ){
      const getImageData=await uploadFileToFirebase(`/ssastore/allcategories/subcategories/${subCategoryField[i].subcategory}/`,subCategoryField[i].subCategoryImage)
