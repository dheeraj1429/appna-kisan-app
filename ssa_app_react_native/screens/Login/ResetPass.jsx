...
const ResetPass = ({ route, navigation }) => {
    ...
    const passwordRegex = /^(?=(.*[a-zA-Z]{2}))(?=.*\d)(?=.*(\*))(?=.*(^))(?=.*(%))(?=.*(#))(?=.*($))(?=.*(@))$/;
    const { phone } = route.params;
    const handleNewPasswordChange = (value) => {
        setNewPassword(value);
        if (value.length < 6) console.log('Min length 6');
        else if (!passwordRegex.test(value)) console.log('Must contain 1 UpperCase, 1 LowerCase, 1 Number, 1 Special Character');
    };
    ...
    const handleChangePassword = async () => {
        ...
        if (newPassword === confirmPassword) {
            try {
                const response = await axios.patch(`${config.BASE_URL}app/update-password `, {
                    "phoneNumber": phone,
