...
//     console.log("VARIABLE=>",category,brand_category)
//   console.log("category-->",category)
  useFocusEffect(
    useCallback(()=>{
        axios.get(`${config.BACKEND_URI}/api/app/get/category/for/new/arrivals/${category}`,{withCredentials:true})
      .then(res=>{
...
useFocusEffect(
  useCallback(()=>{
    ...
    setLoading(true);
    axios.get(`${config.BACKEND_URI}/api/app/get/products/new/arrivals?category=${category}&brand_category=${brand_category == undefined ? '':brand_category}`,{withCredentials:true})
    .then(res=>{
  ...
  const loadMoreItems =async()=>{
   if(currentPage< totalPagesCount){
    ...
    axios.get(`${config.BACKEND_URI}/api/app/get/products/new/arrivals?&page=${currentPage+1}&category=${category}&brand_category=${brand_category == undefined ? '':brand_category}`,{withCredentials:true})
    .then(res=>{
    ...
    const renderbrandSuggestion = useCallback(({item,index})=>{
      return(
       ...
       <Text  style={brand_category != item?.category_name ? {...styles.brandSuggestion} : {...styles.brandSuggestion,color:config.primaryColor}} >{item.category_name}</Text>
       </TouchableOpacity >
      )
