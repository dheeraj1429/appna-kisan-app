...
function RegisterOtp({ route, navigation }) {
  ...
  function onAuthStateChanged(user) {
    if (user) {
      ...
      if (user_exists) {
        ...
        axios
          .post(`${config.BASE_URL}app/verify/register/with/otp`, {
          ...
          .catch((err) => {
            console.log(err, "eror in register otp verification");
            Toast.show({
              type: "success",
              position: "top",
              text1: "Success",
  ...
  const confirmCode = async () => {
    ...
    if (otp.length < 6) {
      Toast.show({
        type: "error",
        position: "top",
        text2: "Please enter 6 characters opt",
        visibilityTime: 4000,
        autoHide: true,
      });
    } else {
  ...
  const loginUser = async () => {
    await axios
      .get(`${config.BACKEND_URI}/api/app/login/user/${phoneNumber}`, {
  ...
  const registerUser = async () => {
    ...
    await axios
      .post(`${config.BACKEND_URI}/api/app/create/user`, data, {
        withCredentials: true,
      })
      .then((res) => {
        console.log(res?.data);
            ...
            <View>
              <View style={styles.commonFieldContainer}>
                <TextInput
                  ...
                  placeholder="Detecting Your Otp*"
                />
                <MaterialIcons
