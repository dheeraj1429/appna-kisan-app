...
function Orders({ navigation }) {
  ...
  const renderItems = ({ item, index }) => {
    return (
      <View key={index} style={{ borderWidth: 0.5, padding: 5, borderRadius: 8, borderColor: 'lightgray' }} >
    ...
    <Provider>
      <Portal>
        <View style={styles.orderMainContainer}>
          ...
          {allOrders?.length <= 0 &&
            <View style={{ marginTop: 80, paddingHorizontal: 100, justifyContent: 'center', alignItems: 'center' }} >
              <Image
          ...
          <Text style={{fontSize:32,color:'black',opacity:100,zIndex:3,opacity:1,paddingVertical:10,backgroundColor:'gray',fontWeight:'700',top:'30%',textAlign:'center'}} >Order Cancelled</Text>
                      ...
                      <View style={styles.orderAndPriceBox}>
                        ...
                        >
                          <TouchableOpacity activeOpacity={0.6} onPress={() => goToViewOrder(item?.order_id, item)} >
                        ...
                        <TouchableOpacity activeOpacity={0.6} onPress={() => goToViewOrder(item?.order_id, item)} style={{ paddingTop: 10 }}>
                          <FlatList
                            horizontal={true}
