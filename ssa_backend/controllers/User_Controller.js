...
const createUser = async(req,res)=>{
    ...
    try{
        const findUserPhone = await Users_Schema.findOne({phone_number:phone_number});
        if(findUserPhone){
...
const loginUser = async(req,res)=>{
    ...
    try{
       ...
       if(!findUserEmail){
         findUserPhone = await Users_Schema.findOne({phone_number:phone_number})
        if(!findUserPhone){
...
const editUserByID = async(req,res)=>{
    ...
    try{
        ...
        if(req.body.password){
            ...
            const find = await Users_Schema.findByIdAndUpdate(userId,{password:hashedNewPassword})
            if(!find){
                return res.send("User not found")
            }
            return res.status(200).send("Password Updated success")
        }
        const findUser = await Users_Schema.findByIdAndUpdate(userId,{$set:req.body})
        if(!findUser){
            return res.send("user not found")
        }
...
const searchInUsers = async(req,res)=>{
    ...
    try{
        ...
        // if(!result.length > 0){
            result = await Users_Schema.find({username:{$regex:searchRegex}}).sort({createdAt:-1})
            if(!result.length > 0){
                result = await Users_Schema.find({email:{$regex:searchRegex}}).sort({createdAt:-1})
            }
        // }
    ...
    try{
        ...
        if(date_from && date_to && by_status ){ 
            ...
            return res.status(200).send(result)
                ...
                result = await Users_Schema.find({verified:user_status},'-password').sort({createdAt:-1})
                // return res.status(200).send(result)
