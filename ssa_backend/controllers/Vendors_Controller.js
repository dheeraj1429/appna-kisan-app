...
const createVendor = async (req, res) => {
  try {
    const findVendorPhone = await Vendors_Schema.findOne({
      shop_contact: req.body.shop_contact,
    });
    if (findVendorPhone) {
      return res.send("User Already Exists !!");
    }
    const findVendor = await Vendors_Schema.findOne({
    ...
    res
      .status(200)
      .send({ result: result, message: "created Vendors successfully !!" });
  } catch (err) {
...
const loginVendor = async (req, res) => {
  ...
  try {
    let findVendorPhone;
    const findVendorEmail = await Vendors_Schema.findOne({
      shop_email: shop_email,
    });
    if (!findVendorEmail) {
      findVendorPhone = await Vendors_Schema.findOne({
...
const getAllVendors = async (req, res) => {
  try {
    ...
    const alluser = await Vendors_Schema.find({}, "-password").sort({
      createdAt: -1,
    });
    res.status(200).json({
      alluser: alluser,
      countVendor: getCountallvendors,
      getAllShopTypes: getAllShopTypes,
      getAllShopPlan: getAllShopPlan,
      getAlldeliveryMethod: getAlldeliveryMethod,
    });
  } catch (err) {
    res.status(500).send("Something went wrong !!");
  }
...
const editVendorByID = async (req, res) => {
  ...
  try {
    ...
    if (req.body.password) {
      ...
      const find = await Vendors_Schema.findByIdAndUpdate(vendors, {
    ...
    const findVendor = await Vendors_Schema.findByIdAndUpdate(vendorId, {
      $set: req.body,
    });
    if (!findVendor) {
...
const searchInVendors = async (req, res) => {
  ...
  try {
    // for string fields
    result = await Vendors_Schema.find({
      shop_id: { $regex: searchRegex },
    }).sort({ createdAt: -1 });
    if (!result.length > 0) {
      result = await Vendors_Schema.find({
    ...
    if (searchValue?.length > 0) {
      ...
      if (!searchForNumber?.length) {
        result = await Vendors_Schema.find({
          shop_contact: JSON.parse(searchValue),
        }).sort({ createdAt: -1 });
      }
    }
...
const filterVendors = async (req, res) => {
  ...
  try {
    ...
    if (date_from && date_to && by_status) {
      ...
      result = await Vendors_Schema.find(
        { shop_status: by_status },
        "-password -account_details -aadhar_card_no -delivery_method -gst_number -profile -shop_address -shop_contact -shop_email -updatedAt -user_type -shop_image"
      ).sort({ createdAt: -1 });
      // return res.status(200).send(result)
    }
    ...
    if (by_delivery_method != "all") {
      result = await Vendors_Schema.find(
        { "delivery_method.name": by_delivery_method },
        "-password -account_details -aadhar_card_no -delivery_method -gst_number -profile -shop_address -shop_contact -shop_email -updatedAt -user_type -shop_image"
      ).sort({ createdAt: -1 });
      return res.status(200).send(result);
    }
