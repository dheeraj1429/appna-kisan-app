...
const brandsSubCategoryAndProducts = async(req,res)=>{
    ...
    try{
        const findBrandSubcategory = await Brands_Schema.find({_id:brandId}).sort({createdAt:-1}).slice({subcategory:7}).limit(7);
        console.log(findBrandSubcategory)
        const findProducts = await Products_Schema.find({product_category:brandName})
        .sort({createdAt:-1})
        .select("product_id product_name new_arrival product_images product_main_category product_category product_subcategory product_variant product_description product_code")
        .limit(10);
        console.log("PRODUCTS=>",findProducts);
        if(findProducts?.length <= 15 ){
            const findMoreProducts = await Products_Schema.find({}).select("product_id product_name new_arrival product_images product_main_category product_category product_subcategory product_variant product_description product_code");
        }
...
const getProductById = async(req,res)=>{
    ...
    try{
        ...
        const findProduct = await Products_Schema.findById(productId).select("product_id color size product_name product_images product_main_category product_category product_subcategory product_variant product_description product_code cartoon_total_products");
        if(!findProduct){
            return res.status(404).send({status:false,message:"product not found !!"});
        }
    ...
    try{
        if(searchBySubCategory){
            ...
            const getAllProductTags = await Products_Schema.aggregate([
                {$match:{product_main_category:searchBySubCategoryObj?.main_category,
                    product_category:searchBySubCategoryObj?.category,
                    product_subcategory:searchBySubCategoryObj?.sub_category
                    }},
...
const getNewArrivalProducts = async(req,res)=>{
    ...
    //  SEARCH FOR NEW ARRIVALS  
        ...
        if(searchForNewArrival?.length && brandCategory != undefined){
            ...
            .select("product_id product_name product_images new_arrival product_main_category product_category product_subcategory product_variant product_description product_code")
            .sort({updatedAt:-1})
            .limit(limit)
            .skip((page - 1) * limit);
        //    console.log("Result Category=>",result)
...
const getBrandCategory=async(req,res)=>{
    ...
    try{
        ...
        const findBrand = await Brands_Schema.find({main_category_name:brandName}).select(" category_name ");
...
const cartCheckout = async(req,res)=>{
    ...
    try{
        ...
        const updateUser = await Users_Schema.findOneAndUpdate({user_id:req.body?.customer_id},
            {$set:{
...
const checkExistingUser = async(req,res)=>{
    ...
    try{
        ...
        const findUser = await Users_Schema.findOne({phone_number:parseInt(phoneNumber)});
        // for sign up 
        if(!findUser){
...
const createUser = async(req,res)=>{
 ...
 const phoneNumber = convertPhone[1]
    try{
        const findUserPhone = await Users_Schema.findOne({phone_number:phoneNumber});
        if(findUserPhone){
        ...
        const result = await create.save();
        const findUser = await Users_Schema.findOne({phone_number:parseInt(phoneNumber)}).select('user_id username phone_number profile')
        if(!findUser){
...
const editUserByID = async(req,res)=>{
    ...
    try{
        ...
        const findUser = await Users_Schema.findOneAndUpdate({_id:userId},
            {$set:{
...
const getAllOrdersByUserId = async(req,res)=>{
    ...
    try{
        ...
        const findOrders = await Order_Schema.find({customer_id:customer_id}).sort({createdAt:-1});
...
const getAllHomeScreenbanner = async(req,res)=>{
    try{
        const findAll = await Banners_Schema.find({}).select("image_url selected_category category_chain").sort({createdAt:-1});
...
const cancelOrderById = async(req,res)=>{
    ...
    try{
        ...
        const findOrder = await Order_Schema.findByIdAndUpdate(orderId,{$set:{order_status:'cancelled'}});
        if(!findOrder){
            return res.status(404).send({status:false,message:'not found !!'});
        }
...
const editUserProfilePicture = async(req,res)=>{
    ...
    try {
        ...
        const updatedUser = await Users_Schema.findByIdAndUpdate(user_id,{$set:{profile:req.body}})
        console.log(updatedUser)
        res.status(200).send({status:true,previousProfile:findUser?.profile,message:'user profile updated sccuess!!'})
...
const getUserProfilePicture = async(req,res)=>{
    ...
    try {
        ...
        res.status(200).send({status:true,profile:findUser?.profile,message:'user profile updated sccuess!!'})
